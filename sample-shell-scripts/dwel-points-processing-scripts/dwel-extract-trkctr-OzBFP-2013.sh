...
# 2. extract ground points from point clouds for DEM generation and then successive registration processing. 
...
$ML "addpath(genpath('$BASEDIR')); EVIPtsPathName='${EVIPTSPATHNAMES[$SGE_TASK_ID-1]}'; EVIPtsFileName='${EVIPTSFILENAMES[$SGE_TASK_ID-1]}'; InPtsPathName='$INPTSPATHNAME'; InPtsFileName='$INPTSFILENAME'; GroundPtsPathName=InPtsPathName; GroundPtsFileName='$GROUNDPTSFILENAME'; run Script1_CvtPtsFmt; run Script2_FilteringVarTIN; GroundPtsPathName='$INPTSPATHNAME'; GroundPtsFileName='$GROUNDPTSFILENAME'; demfilename=fullfile('$INPTSPATHNAME', '$DEMFILENAME'); cellsize=0.2; nodata=-9999; maxTINedgelen=100; run Script3_Points2DEM; ptsfilename=fullfile('$INPTSPATHNAME', '$INPTSFILENAME'); fpfilename=fullfile('$INPTSPATHNAME', '$FPFILENAME'); range = 50; height_clear = 2.0; cellsize = 0.05; maxR = 30; sigmacoef = 1; epsion = 0.01; maxIter = 200; minnumfit = 9; minNumTrkCtr = 4; run Script4_TrkCtrSingleScanLayer;"
