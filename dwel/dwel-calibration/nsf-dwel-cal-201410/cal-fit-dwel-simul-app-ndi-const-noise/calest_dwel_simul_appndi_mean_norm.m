...
% p0 = [p01064(1:4), 0, p01548(1:4), 0, mean([p01064(5), p01548(5)])];
p0 = [p01064(1:4), 0, p01548(1:4), 0];
exitflag = 0
while ~exitflag
...
fprintf(fid, 'exitflag,error,wavelength,c0,c1/c4,c2,c3,eps,b,run_time\n');
fprintf(fid, ['%d,%.3f,%d,',repmat('%.3f,',1,5),'%.3f,%s\n'], ([exitflag, ...
...
export_fig(fullfile(outdir, 'cal_dwel_simul_appndi_const_noise_20140812_1064_mean_norm.png'), '-r300', '-png', '-painters');
figure();
...
export_fig(fullfile(outdir, 'cal_dwel_simul_appndi_const_noise_20140812_1064_mean_norm_fitting_error.png'), '-r300', '-png', '-painters');
% plot scatter plot
...
export_fig(fullfile(outdir, 'cal_dwel_simul_appndi_const_noise_20140812_1064_mean_norm_scatter_plot.png'), '-r300', ...
           '-png', '-painters');
...
title(sprintf(['20140812, 1548, Generalized logistic model fitting to data points \naveraged ' ...
       'at each range locations']));
export_fig(fullfile(outdir, 'cal_dwel_simul_appndi_const_noise_20140812_1548_mean_norm.png'), '-r300', '-png', '-painters');
figure();
...
ylabel('Relative error in return intensity');
legend('(data - model) / data');
export_fig(fullfile(outdir, 'cal_dwel_simul_appndi_const_noise_20140812_1548_mean_norm_fitting_error.png'), '-r300', '-png', '-painters');
% plot scatter plot
...
legend(['R^2=', num2str(r2)], ['slope=', num2str(p(1)), ', intercept=', ...
                   num2str(p(2))], 'Location', 'southeast');
export_fig(fullfile(outdir, 'cal_dwel_simul_appndi_const_noise_20140812_1548_mean_norm_scatter_plot.png'), '-r300', ...
           '-png', '-painters');