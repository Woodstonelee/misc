...
indexfile = ['cal_nsf_20140812_panel_returns_select_index_1064.txt']; 
...
panelrefl = [0.987, 0.5741, 0.4313, 0.041];
...
markerstr = {'o-r', 'o-g', 'o-b', 'o-c'};
ebh = nan(numpanels, 1);
% calculate adjusted reflectance of gray panels based on lambertian panel
% reflectance. 
for pnum=1:numpanels
    x = rg_mean(:, pnum);
    y = int_mean(:, pnum);
...
for pnum=1:numpanels-1
    ...
    if pnum == 1
        normintbase = y*baseint./wireint/panelrefl(pnum);
    end
    adjustrefl(:, pnum) = y*baseint./wireint*panelrefl(1)./normintbase;
