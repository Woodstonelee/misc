...
export_fig('scatter_plot_mean_range_diff_dual_wl_20140812.png', '-r300', '-png', '-painters');
...
for f=1:length(refined1064files)
    ...
    if length(i1064) >= 100
        ...
        % location. 
        diffmean = mean(tmp1064(i1064, 4)-tmp1548(i1548, 4));
        sdmean = std(tmp1064(i1064, 4)-tmp1548(i1548, 4));
        figh = figure('Visible', 'Off');
        [freq, tmpx] = hist(tmp1064(i1064, 4)-tmp1548(i1548, 4), -0.4:0.8/2000:0.4);
        bar(tmpx, freq/sum(freq));
        xlabel('range difference, meter');
        ylabel('probability');
        title(sprintf('Range difference at %.3f meter between 1064 and 1548\n unsaturated single returns from Lambertian panel\nDiff mean=%.3f. Diff SD=%.3f', mean((tmp1064(i1064, 4)+tmp1548(i1548, 4))/2), diffmean, sdmean));
        export_fig(sprintf('hist_lam_range_diff_dual_wl_20140812_%.1f.png', mean((tmp1064(i1064, 4)+tmp1548(i1548, 4))/2)), ...
                   '-r300', '-png', '-painters');
        close(figh);
    end
...
export_fig('scatter_plot_all_lam_range_diff_dual_wl_20140812.png', '-r300', '-png', '-painters');
...
export_fig('hist_lam_range_diff_dual_wl_20140812.png', '-r300', '-png', '-painters');