function [fitp, exitflag, fval] = dwel_simul_app_ndi_fit(data1064, data1548, p0, gaflag, plotflag)
  ...
  % row has the same nominal range position.
      ...
      function err = errFcn(p)
          % p: (c0nir, c1nir, c2nir, c3nir,
          %     c0swir, c1swir, c2swir, c3swir, b)
          kr1064 = gm_func(data1064(:, 1), p(2), p(3), p(4));
          kr1548 = gm_func(data1548(:, 1), p(6), p(7), p(8));

          rhoapp1064 = data1064(:, 3).*data1064(:, 1).^p(9)./kr1064/p(1);
          rhoapp1548 = data1548(:, 3).*data1548(:, 1).^p(9)./kr1548/p(5);
  ...
  if gaflag
    ...
    [fitp, fval, exitflag] = ga(objfunc, 9, [], [], [], [], lb, ub, [], options)
