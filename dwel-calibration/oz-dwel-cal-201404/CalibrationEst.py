...
# this is a user-defined model that sherpa's fitting function can take
  def __init__(self, name='simple_overfilling'):
       self.amplitude = Parameter(name, 'amplitude', 1) # p[0], calibration constant
       self.b = Parameter(name, 'b', 2, min=1, max=3) # p[1], range factor
       self.Ck = Parameter(name,'Ck',1, min=1) # p[2], telescope efficiency parameter
...
# this is a user-defined model that sherpa's fitting function can take
  def __init__(self, name='simple_overfilling'):
       self.amplitude = Parameter(name, 'amplitude', 1) # p[0], calibration constant
       self.Ck = Parameter(name,'Ck', 1, min=1) # p[1], telescope efficiency parameter
...
#    x1=1 # x1 is reflectance, now it's fixed at 1 from Lambertian panel. 
    ret_power=(p[0]*x1/x0**2)*(1-np.array(map(exp,-x0**2/p[1])))
    return ret_power
...
class DWEL_FUN(ArithmeticModel):
  ...
  def calc(self, p, x0, x1, xhi=None, *args, **kwargs):
	  ret_power =(p[0]*x1/x0**2)*(p[1]+x0)**2*(1.-np.exp(-(p[2]*x0)**2/((p[1]+x0)**2)))
	  return ret_power
...
# alternate DWEL model include an extra parameter of range factor
  ...
  def calc(self, p, x0, x1, xhi=None, *args, **kwargs):
	  ret_power =(p[0]*x1/x0**p[3])*(p[1]+x0)**2*(1.-np.exp(-(p[2]*x0)**2/((p[1]+x0)**2)))
	  return ret_power

##############################################################################
# read calibration data from csv file
CalFileName1548 = '/projectnb/echidna/lidar/zhanli86/Programs/CalibrationEstimate/DWEL_Calibration_ESP_1548.csv' # file name of the calibrationd data at 1548 nm. 
caldata1548 = []
...
outputfilename = 'CalibrationEstOutput.txt'
...
print("Fitting to EVI model, three parameters (C_0, b, C_k), Ewan's range test data:")
print evi_fit.format()
print("Fitting to alternate EVI model, two parameters (C_0, C_k), Ewan's range test data:")
print evi_alt_fit.format()
print("Fitting to DWEL model, three parameters (C_0, C_1, C_2), Ewan's range test data:")
print dwel_fit.format()
print("Fitting to alternate DWEL model, four parameters (C_0, C_1, C_2, b), Ewan's range test data:")
print dwel_alt_fit.format()
# write results to output file
outputfile.write("Fitting to EVI model, three parameters (C_0, b, C_k), Ewan's range test data:\n")
outputfile.write(evi_fit.format())
outputfile.write("\n")
outputfile.write("Fitting to alternate EVI model, two parameters (C_0, C_k), Ewan's range test data:\n")
outputfile.write(evi_alt_fit.format())
outputfile.write("\n")
outputfile.write("Fitting to DWEL model, three parameters (C_0, C_1, C_2), Ewan's range test data:\n")
outputfile.write(dwel_fit.format())
outputfile.write("\n")
outputfile.write("Fitting to alternate DWEL model, four parameters (C_0, C_1, C_2, b), Ewan's range test data:\n")
...
plt.plot(range_samples, evi_power_samples, linestyle='-', lw=1, alpha=0.6, color='g',label="EVI three param fit w/ b")
plt.plot(range_samples, evi_alt_power_samples, linestyle=':', lw=1, alpha=0.6, color='g',label="EVI two param fit")
plt.plot(range_samples, dwel_power_samples, linestyle=':', lw=1, alpha=0.6, color='r', label="DWEL three param fit")
plt.plot(range_samples, dwel_alt_power_samples, linestyle='-', lw=1, alpha=0.6, color='r', label="DWEL four param fit w/ b")
plt.xlim(xlim_plt)
plt.ylim(ylim_plt)
...
plt.savefig('calibration_fitting_with_std_error_Ewan.png')
plt.close(fig2)
...
dwel_fit = f_dwel.fit()
dwel_alt_fit = f_dwel_alt.fit()
print("Fitting to EVI model, three parameters (C_0, b, C_k), ESP panel scanning data:")
print evi_fit.format()
print("Fitting to alternate EVI model, two parameters (C_0, C_k), ESP panel scanning data:")
print evi_alt_fit.format()
print("Fitting to DWEL model, three parameters (C_0, C_1, C_2), ESP panel scanning data:")
print dwel_fit.format()
print("Fitting to alternate DWEL model, four parameters (C_0, C_1, C_2, b), ESP panel scanning data:")
print dwel_alt_fit.format()
# write results to output file
outputfile.write("Fitting to EVI model, three parameters (C_0, b, C_k), ESP panel scanning data:\n")
outputfile.write(evi_fit.format())
outputfile.write("\n")
outputfile.write("Fitting to alternate EVI model, two parameters (C_0, C_k), ESP panel scanning data:\n")
outputfile.write(evi_alt_fit.format())
outputfile.write("\n")
outputfile.write("Fitting to DWEL model, three parameters (C_0, C_1, C_2), ESP panel scanning data:\n")
outputfile.write(dwel_fit.format())
outputfile.write("\n")
outputfile.write("Fitting to alternate DWEL model, four parameters (C_0, C_1, C_2, b), ESP panel scanning data:\n")
outputfile.write(dwel_alt_fit.format())
outputfile.write("\n")
...
plt.xlabel("Range",fontsize=10)
plt.ylabel("telescope efficiency changing slope",fontsize=10)
plt.savefig('calibration_fitting_wo_std_error_ESP.png')
plt.close(fig2)
