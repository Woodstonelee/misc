...
outdir = '/usr3/graduate/zhanli86/Programs/misc/dwel-calibration/oz-dwel-cal-201404/cal-simul-appndi-oz-20140425-outputs-v20150201';
...
% ------------------------------------------------------------------------------
% according to David, data within 1.5 m is NOT good for Oz DWEL!!!
% remove them
...
[powerfit1548, ~] = dwel_power_single_fit(nu_meandata1548(nu_meandata1548(:,1)>19.75,:), ...
                      [28701, 2], false);
% p0 = [38445.6, 6580.330, 0.3553, 43.396, 2];
p0 = [powerfit1548(1), 0.151398879, 0.57523766, 62.02558188, powerfit1548(2)];
...
title(sprintf(['20140425, 1064, Generalized logistic model fitting to data points \naveraged ' ...
       'at each range locations']));
export_fig(fullfile(outdir, 'cal_dwel_simul_appndi_20140425_1064_mean_norm.png'), '-r300', '-png', '-painters');
figure();
plot(nu_meandata1064(:, 1), (nu_meandata1064(:,3)-model1064)./nu_meandata1064(:,3), ...
     '.')
title(sprintf(['20140425, 1064, Errors of generalized logistic model fitting to data points \naveraged ' ...
       'at each range locations']));
xlabel('range');
ylabel('Relative error in return intensity');
legend('(data - model) / data');
export_fig(fullfile(outdir, 'cal_dwel_simul_appndi_20140425_1064_mean_norm_fitting_error.png'), '-r300', '-png', '-painters');
% plot scatter plot
...
axis equal;
title(sprintf('20140425, 1064, Scatter plot of measured and modeled return intensity \nraw data points'));
xlabel('measured');
ylabel('modeled');
legend(['$R^2$=', num2str(r2)], ['slope=', num2str(p(1)), ', intercept=', ...
                   num2str(p(2))], 'Location', 'southeast');
export_fig(fullfile(outdir, 'cal_dwel_simul_appndi_20140425_1064_mean_norm_scatter_plot.png'), '-r300', ...
           '-png', '-painters');
...
title(sprintf(['20140425, 1548, Generalized logistic model fitting to data points \naveraged ' ...
       'at each range locations']));
export_fig(fullfile(outdir, 'cal_dwel_simul_appndi_20140425_1548_mean_norm.png'), '-r300', '-png', '-painters');
figure();
plot(nu_meandata1548(:, 1), (nu_meandata1548(:,3)-model1548)./nu_meandata1548(:,3), ...
     '.')
title(sprintf(['20140425, 1548, Errors of generalized logistic model fitting to data points \naveraged ' ...
       'at each range locations']));
xlabel('range');
ylabel('Relative error in return intensity');
legend('(data - model) / data');
export_fig(fullfile(outdir, 'cal_dwel_simul_appndi_20140425_1548_mean_norm_fitting_error.png'), '-r300', '-png', '-painters');
% plot scatter plot
...
axis equal;
title(sprintf('20140425, 1548, Scatter plot of measured and modeled return intensity \nraw data points'));
xlabel('measured');
ylabel('modeled');
legend(['$R^2$=', num2str(r2)], ['slope=', num2str(p(1)), ', intercept=', ...
                   num2str(p(2))], 'Location', 'southeast');
export_fig(fullfile(outdir, 'cal_dwel_simul_appndi_20140425_1548_mean_norm_scatter_plot.png'), '-r300', ...
           '-png', '-painters');