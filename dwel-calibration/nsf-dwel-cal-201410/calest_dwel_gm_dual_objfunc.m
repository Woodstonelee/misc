...
% data1064 or data1548: (range, refls, return_I)
	...
	ret_model1064 = (p(1)*data1064(:, 2)./data1064(:, 1).^p(9)).*(ones(size(data1064(:,1)))+p(2)*exp(-1*p(3)*(data1064(:, 1)+p(4)*ones(size(data1064(:, 1)))))).^(-1*p(3))
	ret_model1548 = (p(5)*data1548(:, 2)./data1548(:, 1).^p(9)).*(ones(size(data1548(:,1)))+p(6)*exp(-1*p(7)*(data1548(:, 1)+p(8)*ones(size(data1548(:, 1)))))).^(-1*p(7))        
