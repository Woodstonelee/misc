function [fitp, exitflag, fval] = dwel_simul_app_ndi_fit_step2(data1064, data1548, p0, fixp, gaflag, plotflag)

  function h = calest_dwel_gm_single_objfunc(data1064, data1548, fixp)
  ...
  % fixp: c1nir/c1swir, c3nir/c3swir
      h = @errFcn;
      % nested, i.e. objective function.
      function err = errFcn(p)
          % p: (c0nir, c2nir, bnir
          %     c0swir, c2swir, bswir)
          kr1064 = gm_func(data1064(:, 1), fixp(1), p(2), fixp(2));
          kr1548 = gm_func(data1548(:, 1), fixp(1), p(5), fixp(2));

          rhoapp1064 = data1064(:, 3).*data1064(:, 1).^p(3)./kr1064/p(1);
          rhoapp1548 = data1548(:, 3).*data1548(:, 1).^p(6)./kr1548/p(4);
          ...
          % F-ratio, model
          f_ratio_model = (rhoapp1064./rhoapp1548)./(data1064(:, 3)./data1548(:, 3));
          f_ratio_true = (data1064(:, 2)./data1548(:, 2))./(data1064(:, 3)./data1548(:, 3));
          f_ratio_err = sum((f_ratio_model - f_ratio_true).^2);

          % total error to minimize
          % err = rhoapperr + ndierr + avgrhoapperr + f_ratio_err;
          err = rhoapperr + avgrhoapperr + f_ratio_err;
      end
      % nested function
  end

  % create the objective function
  objfunc = calest_dwel_gm_single_objfunc(data1064, data1548, fixp);

  if gaflag
    % Now use genetic algorithm (GA) to search the global minimum. 
    % We have 9 parameters in total now. 
    lb = [0, 0, 1, 0, 0, 1];
    ub = [Inf, Inf, 3, Inf, Inf, 3];
